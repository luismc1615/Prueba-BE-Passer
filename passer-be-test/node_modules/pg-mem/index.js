...
function errorMessage(error) {
    ...
    return (_c = error) === null || _c === void 0 ? void 0 : _c.message;
}
...
const restrictive_index_1 = __webpack_require__(47);
let fns;
...
const interfaces_1 = __webpack_require__(2);
const object_hash_1 = __importDefault(__webpack_require__(14));
...
const select_1 = __webpack_require__(15);
...
function _buildValue(val) {
    ...
    const data = context_1.buildCtx().selection;
...
function _buildValueReal(val) {
    var _a;
    const { schema, getParameter, selection } = context_1.buildCtx();
    switch (val.type) {
        case 'binary':
        ...
        case 'ref':
            ...
            throw new interfaces_1.ColumnNotFound(utils_1.colToStr(val));
        case 'string':
        ...
        case 'call':
            ...
            if (nm && aggregation_1.aggregationFunctions.has(nm)) {
                const agg = aggregation_1.getAggregator();
                if (!agg) {
...
function buildBinaryValue(leftValue, op, rightValue) {
    ...
    switch (op) {
        ...
        case 'NOT ILIKE':
            ...
            break;
        default: {
            const { schema } = context_1.buildCtx();
...
function buildCase(op) {
    ...
    return new evaluator_1.Evaluator(valueType, null, object_hash_1.default({ when: whenExprs.map(x => ({ when: x.when.hash, then: x.then.hash })) }), [
...
function buildMember(op) {
    ...
    return new evaluator_1.Evaluator(op.op === '->' ? onExpr.type : datatypes_1.Types.text(), null, object_hash_1.default([onExpr.hash, op.op, op.member]), onExpr, typeof op.member === 'string'
        ? (raw, t) => {
...
function buildArrayIndex(op) {
    ...
    return new evaluator_1.Evaluator(onExpr.type.of, null, object_hash_1.default({ array: onExpr.hash, index: index.hash }), [onExpr, index], (raw, t) => {
        const value = onExpr.get(raw, t);
        if (!Array.isArray(value)) {
            return null;
        }
...
function buildTernary(op) {
    ...
    const conv = oop === 'NOT BETWEEN'
        ? (x) => !x
        : (x) => x;
    return new evaluator_1.Evaluator(datatypes_1.Types.bool, null, object_hash_1.default({ value: value.hash, lo: lo.hash, hi: hi.hash }), [value, hi, lo], (raw, t) => {
...
function buildSelectAsArray(op) {
    ...
    const onData = select_1.buildSelect(op);
...
function buildExtract(op) {
    ...
    function extract(as, fn, result = datatypes_1.Types.integer) {
        ...
        return new evaluator_1.Evaluator(result, null, object_hash_1.default({ extract: from.hash, field: op.field }), [conv], (raw, t) => {
...
function buildOverlay(op) {
    ...
    return new evaluator_1.Evaluator(datatypes_1.Types.text(), null, object_hash_1.default({ overlay: value.hash, placing: placing.hash, from: from.hash, for: forr === null || forr === void 0 ? void 0 : forr.hash }), forr ? [value, placing, from, forr] : [value, placing, from], (raw, t) => {
...
function buildSubstring(op) {
    ...
    return new evaluator_1.Evaluator(datatypes_1.Types.text(), null, object_hash_1.default({ substr: value.hash, from: from === null || from === void 0 ? void 0 : from.hash, for: forr === null || forr === void 0 ? void 0 : forr.hash }), vals, (raw, t) => {
        const _value = value.get(raw, t);
        if (utils_1.nullIsh(_value)) {
            return null;
        }
...
const evaluator_1 = __webpack_require__(10);
const interfaces_private_1 = __webpack_require__(0);
const datatypes_1 = __webpack_require__(13);
const utils_1 = __webpack_require__(1);
const object_hash_1 = __importDefault(__webpack_require__(14));
const interfaces_1 = __webpack_require__(2);
let regCnt = 0;
...
class TypeBase {
    ...
    equals(a, b) {
        if (utils_1.nullIsh(a) || utils_1.nullIsh(b)) {
            return null;
        }
    ...
    canCast(to) {
        ...
        if ((_d = this.doCanCast) === null || _d === void 0 ? void 0 : _d.call(this, to)) {
            return true;
        }
        return false;
    }
...
const alias_1 = __webpack_require__(24);
...
const object_hash_1 = __importDefault(__webpack_require__(14));
const datatypes_1 = __webpack_require__(3);
const function_call_1 = __webpack_require__(37);
const utils_1 = __webpack_require__(1);
class Evaluator {
    ...
    setConversion(converter, hashConv) {
        return new Evaluator(this.type, this.id, object_hash_1.default(hashConv(this.hash)), this, (raw, t) => {
...
exports.Value = {
    ...
    function(value, args) {
        return function_call_1.buildCall(value, args);
    },
    bool(value) {
    ...
    constant(_type, value) {
        const type = utils_1.nullIsh(value)
    ...
    in(value, array, inclusive) {
        ...
        return new Evaluator(datatypes_1.Types.bool, null, object_hash_1.default({ val: value.hash, in: array.hash }), [value, array], (raw, t) => {
    ...
    isNull(leftValue, expectNull) {
        return new Evaluator(datatypes_1.Types.bool, null, object_hash_1.default({ isNull: leftValue.hash, expectNull }), leftValue, (raw, t) => {
            const left = leftValue.get(raw, t);
            // check that result is null (will never return NULL)
            const ret = utils_1.nullIsh(left);
            return expectNull ? ret : !ret;
        });
    },
    isTrue(leftValue, expectTrue) {
        leftValue = leftValue.cast(datatypes_1.Types.bool);
        return new Evaluator(datatypes_1.Types.bool, null, object_hash_1.default({ isTrue: leftValue.hash, expectTrue }), leftValue, expectTrue ? ((raw, t) => {
            const left = leftValue.get(raw, t);
    ...
    isFalse(leftValue, expectFalse) {
        leftValue = leftValue.cast(datatypes_1.Types.bool);
        return new Evaluator(datatypes_1.Types.bool, null, object_hash_1.default({ isFalse: leftValue.hash, expectFalse }), leftValue, expectFalse ? ((raw, t) => {
            const left = leftValue.get(raw, t);
...
function arrayOrList(values, list) {
    ...
    return new Evaluator(list ? type.asList() : type.asArray(), null, object_hash_1.default(converted.map(x => x.hash)), converted, (raw, t) => {
...
function buildParameterList(statementName, args) {
    return args.map(({ type, name }, i) => {
        const value = new Evaluator(type, name, object_hash_1.default({ param: name }), null, () => {
            var _a;
            const { parametersValues } = utils_1.executionCtx();
...
function buildSelection(on, select) {
    ...
    for (const col of select !== null && select !== void 0 ? select : []) {
        if (!(on instanceof aggregation_1.Aggregation) && 'expr' in col && hasAggreg(col.expr)) {
...
function* buildCols(base, columns) {
    ...
    for (const s of columns) {
        ...
        if (s.expr.type === 'ref' && s.expr.name === '*') {
            ...
            const val = expression_builder_1.buildValue(s.expr);
...
class Selection extends transform_base_1.TransformBase {
    constructor(base, _columns) {
        ...
        const columns = context_1.withSelection(base, () => [...buildCols(base, _columns)]);
...
const buffer_node_1 = __webpack_require__(21);
const datatype_base_1 = __webpack_require__(8);
const datatypes_geometric_1 = __webpack_require__(38);
const t_interval_1 = __webpack_require__(39);
const t_time_1 = __webpack_require__(40);
const t_timestamp_1 = __webpack_require__(41);
const t_jsonb_1 = __webpack_require__(42);
const t_regtype_1 = __webpack_require__(43);
const t_regclass_1 = __webpack_require__(44);
const t_record_1 = __webpack_require__(19);
const t_inet_1 = __webpack_require__(45);
const context_1 = __webpack_require__(4);
...
class ArrayType extends datatype_base_1.TypeBase {
    ...
    toText(to, value) {
        ...
        return new evaluator_1.Evaluator(to, value.id, value.hash, value, (raw, t) => {
            const arr = value.get(raw, t);
            const strs = arr.map(x => evaluator_1.Value.constant(valueType.of, x).cast(exports.Types.text()).get(raw, t));
...
function makeTimestamp(primary, len = null) {
    ...
    if (!got) {
        timestamps.set(len, got = new t_timestamp_1.TimestampType(primary, len));
    }
    return got;
}
...
const interfaces_private_1 = __webpack_require__(0);
const deletion_1 = __webpack_require__(53);
const update_1 = __webpack_require__(54);
const insert_1 = __webpack_require__(55);
const values_table_1 = __webpack_require__(56);
const utils_1 = __webpack_require__(1);
const join_1 = __webpack_require__(28);
const clean_results_1 = __webpack_require__(18);
...
const datatypes_1 = __webpack_require__(3);
const t_record_1 = __webpack_require__(19);
const function_call_table_1 = __webpack_require__(57);
...
function buildWith(p, topLevel) {
    return context_1.withBindingScope(() => {
        const { setTempBinding } = context_1.buildCtx();
    ...
    for (const from of (_a = p.from) !== null && _a !== void 0 ? _a : []) {
        ...
        switch (from.type) {
            ...
            case 'call':
                ...
                const fromValue = expression_builder_1.buildValue(from);
                if (datatypes_1.ArrayType.matches(fromValue.type) && t_record_1.RecordType.matches(fromValue.type.of)) {
                    ...
                    newT = new values_table_1.ValuesTable(fnName, [[from]], [fnName])
                        .setAlias((_g = (_f = (_e = from.alias) === null || _e === void 0 ? void 0 : _e.name) !== null && _f !== void 0 ? _f : utils_1.suggestColumnName(from)) !== null && _g !== void 0 ? _g : '');
                }
                break;
            default:
        ...
        switch ((_h = from.join) === null || _h === void 0 ? void 0 : _h.type) {
            ...
            case 'RIGHT JOIN':
                ...
                break;
            default:
                throw new interfaces_private_1.NotSupported('Joint type not supported ' + ((_k = (_j = from.join) === null || _j === void 0 ? void 0 : _j.type) !== null && _k !== void 0 ? _k : '<no join specified>'));
        }
    }
    // filter & select
    sel = sel !== null && sel !== void 0 ? sel : context_1.buildCtx().schema.dualTable.selection;
...
function getSelectable(name) {
    const { schema, getTempBinding } = context_1.buildCtx();
    const temp = !name.schema
        && getTempBinding(name.name);
    let ret = temp || interfaces_private_1.asSelectable(schema.getObject(name)).selection;
...
class SelectExec {
    ...
    execute(t) {
        var _a;
        const rows = clean_results_1.cleanResults([...this.selection.enumerate(t)]);
        return {
            result: {
                rows,
                ...
                fields: [],
                location: exec_utils_1.locOf(this.p),
            },
            state: t,
        };
    }
...
class RecordType extends datatype_base_1.TypeBase {
    ...
    transformItemFrom(source) {
        ...
        return (raw, t, execId) => {
            ...
            interfaces_private_1.setId(ret, execId + interfaces_private_1.getId(raw));
...
const transform_base_1 = __webpack_require__(5);
const array_filter_1 = __webpack_require__(27);
const clean_results_1 = __webpack_require__(18);
const selection_1 = __webpack_require__(11);
const expression_builder_1 = __webpack_require__(6);
const context_1 = __webpack_require__(4);
...
class MutationDataSourceBase extends transform_base_1.DataSourceBase {
    constructor(table, mutatedSel, p) {
        ...
        if (p.returning) {
            ...
            this.returning = selection_1.buildSelection(this.returningRows, p.returning);
        }
        const { onFinishExecution } = context_1.buildCtx();
    ...
    *enumerate(t) {
        ...
        try {
            clean_results_1.cleanResults(affected);
    ...
    getColumn(column, nullIfNotFound) {
        if (!this.returning) {
            throw new Error(`Cannot get column "${utils_1.colToStr(column)}" from a mutation that has no returning statement`);
        }
...
function createSetter(setTable, setSelection, _sets) {
    return context_1.withSelection(setSelection, () => {
        const sets = _sets.map(x => {
            ...
            return {
                ...
                getter: x.value.type !== 'default'
                    ? expression_builder_1.buildValue(x.value).cast(col.expression.type)
                    : null,
            };
        });
...
const object_hash_1 = __importDefault(__webpack_require__(14));
const evaluator_1 = __webpack_require__(10);
const count_1 = __webpack_require__(48);
const max_min_1 = __webpack_require__(49);
const sum_1 = __webpack_require__(50);
const array_agg_1 = __webpack_require__(51);
const avg_1 = __webpack_require__(52);
const selection_1 = __webpack_require__(11);
const context_1 = __webpack_require__(4);
...
function getAggregator() {
    const on = context_1.buildCtx().selection;
    if (!on) {
        return null;
    }
...
class Aggregation extends transform_base_1.TransformBase {
    constructor(on, _groupedBy) {
        ...
        this.groupingValuesOnbase = context_1.withSelection(on, () => _groupedBy.map(x => expression_builder_1.buildValue(x)));
    ...
    *enumerate(t) {
        for (const item of this._enumerateAggregationKeys(t)) {
            ...
            interfaces_private_1.setId(item, `agg_${this.aggId}_${object_hash_1.default(sym)}`);
            yield item;
        }
    }
...
function parseSql(sql, entry) {
    ...
    try {
        ...
        const msg = utils_1.errorMessage(e);
...
class JoinSelection extends transform_base_1.DataSourceBase {
    constructor(restrictive, joined, on, innerJoin) {
        super(context_1.buildCtx().schema);
    ...
    fetchOnStrategies(_on) {
        ...
        for (const on of extractAnds(_on)) {
            if (on.type !== 'binary' || on.op !== '=') {
                ...
                others.push(expression_builder_1.buildValue(on));
                continue;
            }
            this.building = true;
            const left = expression_builder_1.buildValue(on.left);
            const right = expression_builder_1.buildValue(on.right);
            this.building = false;
            ...
            ands.push({
                left,
                right,
                eq: expression_builder_1.buildValue(on),
            });
        }
        // compute strategies
        this.fetchAndStrategies(ands, others);
        // build seq-scan expression
        this.seqScanExpression = expression_builder_1.buildValue(_on).cast(datatypes_1.Types.bool);
    }
    fetchUsingStrategies(_using) {
        ...
        const ands = _using.map(n => {
            ...
            return {
                left,
                right,
                eq: expression_builder_1.buildBinaryValue(this.wrap(left), '=', this.wrap(right))
            };
        });
        ...
        this.seqScanExpression = ands.slice(1)
            .reduce((a, b) => expression_builder_1.buildBinaryValue(a, 'AND', b.eq), ands[0].eq);
    }
        ...
        for (let i = 0; i < ands.length; i++) {
            ...
            if (others.length) {
                const and = others.slice(1)
                    .reduce((v, c) => expression_builder_1.buildBinaryValue(c, 'AND', v), others[0]);
...
const interfaces_private_1 = __webpack_require__(0);
const sequence_1 = __webpack_require__(67);
const exec_utils_1 = __webpack_require__(7);
class ExecuteCreateSequence extends exec_utils_1.ExecHelper {
    ...
    createSeq(t) {
        ...
        let ret = null;
        exec_utils_1.checkExistence(this.schema, name, p.ifNotExists, () => {
...
const interfaces_private_1 = __webpack_require__(0);
const pgsql_ast_parser_1 = __webpack_require__(12);
const create_table_1 = __webpack_require__(68);
const create_sequence_1 = __webpack_require__(30);
const exec_utils_1 = __webpack_require__(7);
const create_index_1 = __webpack_require__(69);
const alter_1 = __webpack_require__(70);
const alter_sequence_1 = __webpack_require__(71);
const drop_index_1 = __webpack_require__(72);
const drop_table_1 = __webpack_require__(73);
const drop_sequence_1 = __webpack_require__(74);
const transaction_statements_1 = __webpack_require__(75);
const truncate_table_1 = __webpack_require__(76);
const show_1 = __webpack_require__(77);
const set_1 = __webpack_require__(78);
const create_enum_1 = __webpack_require__(79);
const create_view_1 = __webpack_require__(80);
const create_materialized_view_1 = __webpack_require__(81);
const create_schema_1 = __webpack_require__(82);
const create_function_1 = __webpack_require__(83);
const do_1 = __webpack_require__(84);
const select_1 = __webpack_require__(15);
const context_1 = __webpack_require__(4);
...
class StatementExec {
    ...
    compile() {
        return this.niceErrors(() => {
            ...
            if (!this.db.options.noAstCoverageCheck) {
                const watched = utils_1.watchUse(p);
                p = watched.checked;
            ...
            const namedParams = utils_1.fromEntries((_b = (_a = this.parameters) === null || _a === void 0 ? void 0 : _a.filter(p => !!p.value.id).map(x => [x.value.id, x])) !== null && _b !== void 0 ? _b : []);
            const nameResolver = new MapNameResolver(namedParams, true);
            // parse the AST
            context_1.withNameResolver(nameResolver, () => context_1.withStatement(this, () => context_1.withSelection(this.schema.dualTable.selection, () => this.executor = this._getExecutor(p))));
            return this.executor;
        });
    }
    executeStatement(t) {
        return this.niceErrors(() => utils_1.pushExecutionCtx({
            transaction: t,
            schema: this.schema,
        }, () => {
            var _a;
    ...
    but the resulting statement cannot be executed → Probably not a pg-mem error.`);
                ...
                if (!this.db.options.noErrorDiagnostic) {
                    if (this.pAsSql) {
                        ...
                        try {
                            ...
                            msgs.push(`*️⃣ <Failed to reconsitute SQL - ${utils_1.errorMessage(f)}>`);
                        }
                    }
                }
            ...
            if (e && typeof e === 'object') {
                e.location = exec_utils_1.locOf(this.statement);
...
const interfaces_1 = __webpack_require__(2);
const interfaces_private_1 = __webpack_require__(0);
const schema_1 = __webpack_require__(36);
const transform_base_1 = __webpack_require__(5);
const selection_1 = __webpack_require__(11);
const alias_1 = __webpack_require__(24);
const build_filter_1 = __webpack_require__(85);
const adapters_1 = __webpack_require__(95);
const transaction_1 = __webpack_require__(98);
const aggregation_1 = __webpack_require__(23);
const limit_1 = __webpack_require__(99);
const union_1 = __webpack_require__(100);
const distinct_1 = __webpack_require__(101);
const order_by_1 = __webpack_require__(102);
const pg_catalog_1 = __webpack_require__(103);
const information_schema_1 = __webpack_require__(119);
const utils_1 = __webpack_require__(1);
function newDb(opts) {
...
const utils_1 = __webpack_require__(1);
const datatypes_1 = __webpack_require__(3);
const table_1 = __webpack_require__(46);
const parse_cache_1 = __webpack_require__(25);
const migrate_1 = __webpack_require__(63);
const t_custom_enum_1 = __webpack_require__(64);
const datatype_base_1 = __webpack_require__(8);
const t_equivalent_1 = __webpack_require__(65);
const overload_resolver_1 = __webpack_require__(66);
const create_sequence_1 = __webpack_require__(30);
const statement_exec_1 = __webpack_require__(31);
const select_1 = __webpack_require__(15);
class DbSchema {
    constructor(name, db) {
        this.name = name;
    ...
    dropFunction(fn) {
        ...
        if (fn.arguments) {
            ...
            if (!(match === null || match === void 0 ? void 0 : match.length)) {
                ...
                throw new interfaces_1.QueryError(`function ${fn.name.name}(${targetArgs.map(t => interfaces_1.typeDefToStr(t.type)).join(',')}) does not exist`, '42883');
            }
            if (match.length > 1) {
...
function buildCall(name, args) {
    ...
    return new evaluator_1.Evaluator(type !== null && type !== void 0 ? type : datatypes_1.Types.null, null, object_hash_1.default({ call: name, args: args.map(x => x.hash) }), args, (raw, t) => {
...
function buildAnyCall(args) {
    ...
    if (array.type instanceof datatypes_1.ArrayType) {
        return new evaluator_1.Evaluator(array.type.of, null, object_hash_1.default({ any: array.hash }), args, (raw, t) => {
    ...
    return new evaluator_1.Evaluator(datatypes_1.Types.text(), null, object_hash_1.default({ any: array.hash }), args, arrayValue, { isAny: true } // <== isAny !
    );
}
                ...
                return value
                    ...
                    try {
                        ...
                        throw new interfaces_1.QueryError({
                            ...
                            details: utils_1.errorMessage(e),
                            code: '22P02',
                        });
                    }
                }, toJsonb => ({ toJsonb }));
...
const interfaces_private_1 = __webpack_require__(0);
const expression_builder_1 = __webpack_require__(6);
const btree_index_1 = __webpack_require__(58);
const selection_1 = __webpack_require__(11);
const utils_1 = __webpack_require__(1);
const immutable_1 = __webpack_require__(16);
const column_1 = __webpack_require__(60);
const alias_1 = __webpack_require__(24);
const transform_base_1 = __webpack_require__(5);
const foreign_key_1 = __webpack_require__(62);
const datatypes_1 = __webpack_require__(3);
const context_1 = __webpack_require__(4);
class ColumnManager {
    constructor() {
...
class MemoryTable extends transform_base_1.DataSourceBase {
    ...
    *enumerate(t) {
    ...
    addCheck(_t, check, constraintName) {
        ...
        const getter = context_1.withSelection(this.selection, () => expression_builder_1.buildValue(check).cast(datatypes_1.Types.bool));
        const checkVal = (t, v) => {
...
function buildCount(base, call) {
    return context_1.withSelection(base, () => {
        ...
        if (call.distinct) {
            ...
            if (args.length === 1 && args[0].type === 'list') {
                ...
                const distinctArgs = args[0].expressions.map(x => expression_builder_1.buildValue(x));
                return new CountDistinct(distinctArgs);
            }
            else {
                const distinctArgs = args.map(x => expression_builder_1.buildValue(x));
                return new CountDistinct(distinctArgs);
            }
        }
        const what = expression_builder_1.buildValue(args[0]);
...
function buildSum(base, call) {
    return context_1.withSelection(base, () => {
        ...
        if (call.distinct) {
            ...
            const distinctArg = expression_builder_1.buildValue(args[0]);
...
class Deletion extends mutation_base_1.MutationDataSourceBase {
    constructor(ast) {
        const { schema } = context_1.buildCtx();
        const table = interfaces_private_1.asTable(schema.getObject(ast.from));
...
class Update extends mutation_base_1.MutationDataSourceBase {
    constructor(ast) {
        const { schema } = context_1.buildCtx();
        const into = interfaces_private_1.asTable(schema.getObject(ast.table));
        const mutatedSel = into
            .selection
            .filter(ast.where);
        super(into, mutatedSel, ast);
        this.setter = mutation_base_1.createSetter(this.table, this.mutatedSel, ast.sets);
    }
    performMutation(t) {
        // perform update
        const rows = [];
...
class Insert extends mutation_base_1.MutationDataSourceBase {
    constructor(ast) {
        ...
        const table = interfaces_private_1.asTable(schema.getObject(ast.into));
        ...
        this.valueRawSource = ast.insert.type === 'values'
            ? select_1.buildValues(ast.insert, true)
            : select_1.buildSelect(ast.insert);
        context_1.withSelection(this, () => this.visit(ast));
    }
    visit(ast) {
        var _a, _b;
        ...
        if (ast.onConflict) {
            ...
            const on = _on && context_1.withSelection(this.table.selection, () => _on === null || _on === void 0 ? void 0 : _on.map(x => expression_builder_1.buildValue(x)));
            let onIndex = null;
            if (on) {
            ...
            if (ast.onConflict.do === 'do nothing') {
                ...
                const setter = mutation_base_1.createSetter(this.table, subject, ast.onConflict.do.sets);
                const _where = ast.onConflict.where;
                const where = _where && context_1.withSelection(subject, () => expression_builder_1.buildValue(_where));
                ignoreConflicts = {
                    onIndex,
...
const interfaces_1 = __webpack_require__(2);
const interfaces_private_1 = __webpack_require__(0);
const readonly_table_1 = __webpack_require__(9);
const expression_builder_1 = __webpack_require__(6);
const datatypes_1 = __webpack_require__(3);
const context_1 = __webpack_require__(4);
let cnt = 0;
class ValuesTable extends readonly_table_1.ReadOnlyTable {
    constructor(alias, items, columnNames, acceptDefault) {
        super(context_1.buildCtx().schema);
        this.symbol = Symbol();
        context_1.withSelection(context_1.buildCtx().schema.dualTable.selection, () => {
            ...
            let builtVals = items.map(vals => vals.map(e => {
                ...
                return expression_builder_1.buildValue(e);
            }));
...
class FunctionCallTable extends transform_base_1.DataSourceBase {
    constructor(cols, evaluator) {
        ...
        this.symbol = Symbol();
        this.columns = cols.map(c => selection_1.columnEvaluator(this, c.name, c.type).setOrigin(this));
        this.colsByName = utils_1.fromEntries(this.columns.map(c => [c.id, c]));
    }
    get isExecutionWithNoResult() {
        return false;
    }
...
const interfaces_private_1 = __webpack_require__(0);
// @ts-ignore
const functional_red_black_tree_1 = __importDefault(__webpack_require__(59));
const interfaces_1 = __webpack_require__(2);
const immutable_1 = __webpack_require__(16);
const utils_1 = __webpack_require__(1);
class BIndex {
    ...
    compare(_a, _b) {
        for (let i = 0; i < this.expressions.length; i++) {
            ...
            const a = _a[i];
            const b = _b[i];
            const an = utils_1.nullIsh(a);
            const bn = utils_1.nullIsh(b);
            if (an || bn) {
                if (an === bn) {
                    continue;
                }
    ...
    add(raw, t) {
        ...
        if (this.predicate) {
            ...
            if (utils_1.nullIsh(val) || val === false) {
                return;
            }
        }
        ...
        const hasNil = utils_1.hasNullish(...key);
...
const utils_1 = __webpack_require__(1);
const selection_1 = __webpack_require__(11);
const generated_1 = __webpack_require__(61);
const expression_builder_1 = __webpack_require__(6);
const context_1 = __webpack_require__(4);
class ColRef {
    ...
    alter(alter, t) {
        ...
        switch (alter.type) {
            ...
            case 'set default':
                ...
                const df = context_1.withSelection(this.table.selection, () => expression_builder_1.buildValue(alter.default));
                if (!df.isConstant) {
...
const pg_escape_1 = __webpack_require__(29);
...
class OverloadNode {
    ...
    *all() {
    ...
    resolve(args) {
        ...
        const sigsToCheck = utils_1.it((_a = this.nexts // perf tweak: search by primary type
            .get(arg.type.primary)) !== null && _a !== void 0 ? _a : utils_1.it(this.nexts.values()).flatten() // else, search all registered overloads
        );
...
class ExecuteCreateTable extends exec_utils_1.ExecHelper {
    ...
    execute(t) {
        ...
        t = t.fullCommit();
        // perform creation
        exec_utils_1.checkExistence(this.schema, this.name, this.ifNotExists, () => {
...
class CreateIndexExec extends exec_utils_1.ExecHelper {
    constructor({ schema }, p) {
        ...
        this.onTable = interfaces_private_1.asTable(schema.getObject(p.table));
        ...
        this.indexDef = context_1.withSelection(this.onTable.selection, () => {
            // index columns
            const columns = p.expressions
                .map(x => {
                return {
                    value: expression_builder_1.buildValue(x.expression),
                    nullsLast: x.nulls === 'last',
                    desc: x.order === 'desc',
                };
            });
            // compile predicate (if any)
            const predicate = p.where && expression_builder_1.buildValue(p.where);
            return {
                columns,
                indexName,
                unique: p.unique,
...
class Alter extends exec_utils_1.ExecHelper {
    constructor({ schema }, p) {
        super(p);
        this.p = p;
        this.table = interfaces_private_1.asTable(schema.getObject(p.table));
...
class AlterSequence extends exec_utils_1.ExecHelper {
    constructor({ schema }, p) {
        super(p);
        this.p = p;
        this.seq = interfaces_private_1.asSeq(schema.getObject(p.name, {
            nullIfNotFound: p.ifExists,
        }));
        if (!this.seq) {
            utils_1.ignore(this.p);
        }
    }
    execute(t) {
        var _a;
...
class DropIndex extends exec_utils_1.ExecHelper {
    constructor({ schema }, statement) {
        super(statement);
        this.idx = interfaces_private_1.asIndex(schema.getObject(statement.name, {
            nullIfNotFound: statement.ifExists,
        }));
        if (this.idx) {
            utils_1.ignore(statement.concurrently);
        }
        else {
            utils_1.ignore(statement);
        }
    }
    execute(t) {
        var _a;
        ...
        t = t.fullCommit();
        // alter the sequence
        (_a = this.idx) === null || _a === void 0 ? void 0 : _a.onTable.dropIndex(t, this.idx.name);
        // new implicit transaction
        t = t.fork();
...
class DropTable extends exec_utils_1.ExecHelper {
    constructor({ schema }, statement) {
        super(statement);
        this.table = interfaces_private_1.asTable(schema.getObject(statement.name, {
            nullIfNotFound: statement.ifExists,
        }));
        if (!this.table) {
            utils_1.ignore(statement);
    ...
    execute(t) {
        ...
        t = t.fullCommit();
        // drop table
        (_a = this.table) === null || _a === void 0 ? void 0 : _a.drop(t);
        // new implicit transaction
        t = t.fork();
        return this.noData(t, 'DROP');
    }
...
class DropSequence extends exec_utils_1.ExecHelper {
    constructor({ schema }, statement) {
        super(statement);
        this.seq = interfaces_private_1.asSeq(schema.getObject(statement.name, {
            nullIfNotFound: statement.ifExists,
        }));
        if (!this.seq) {
            utils_1.ignore(statement);
    ...
    execute(t) {
        ...
        t = t.fullCommit();
        // drop the sequence
        (_a = this.seq) === null || _a === void 0 ? void 0 : _a.drop(t);
        // new implicit transaction
        t = t.fork();
        return this.noData(t, 'DROP');
    }
...
class TruncateTable extends exec_utils_1.ExecHelper {
    constructor(statement) {
        ...
        const { schema } = context_1.buildCtx();
        this.table = interfaces_private_1.asTable(schema.getObject(statement.tables[0]));
    }
    execute(t) {
...
const exec_utils_1 = __webpack_require__(7);
const utils_1 = __webpack_require__(1);
const view_1 = __webpack_require__(32);
const select_1 = __webpack_require__(15);
class CreateView extends exec_utils_1.ExecHelper {
    constructor(st, p) {
        ...
        // check existence
        this.existing = interfaces_private_1.asView(this.schema.getObject(p.name, { nullIfNotFound: true }));
        utils_1.ignore(p.orReplace);
        this.drop = !!(p.orReplace && this.existing);
        let view = select_1.buildSelect(p.query);
...
class CreateMaterializedView extends exec_utils_1.ExecHelper {
    constructor(st, p) {
        ...
        const view = select_1.buildSelect(p.query);
...
class CreateFunction extends exec_utils_1.ExecHelper {
    constructor({ schema }, fn) {
        ...
        // determine arg types
        const args = context_1.withSelection(schema.dualTable.selection, () => fn.arguments.map(a => {
            var _a;
            return ({
                name: (_a = a.name) === null || _a === void 0 ? void 0 : _a.name,
                type: schema.getType(a.type),
                default: a.default && expression_builder_1.buildValue(a.default),
                mode: a.mode,
            });
        }));
...
const expression_builder_1 = __webpack_require__(6);
const datatypes_1 = __webpack_require__(3);
const eq_filter_1 = __webpack_require__(86);
const evaluator_1 = __webpack_require__(10);
const false_filter_1 = __webpack_require__(87);
const and_filter_1 = __webpack_require__(88);
const or_filter_1 = __webpack_require__(89);
const seq_scan_1 = __webpack_require__(33);
const in_filter_1 = __webpack_require__(90);
const not_in_filter_1 = __webpack_require__(91);
const startswith_filter_1 = __webpack_require__(92);
const ineq_filter_1 = __webpack_require__(93);
const utils_1 = __webpack_require__(1);
const between_filter_1 = __webpack_require__(94);
const interfaces_1 = __webpack_require__(2);
const context_1 = __webpack_require__(4);
function buildFilter(on, filter, parentName) {
    return context_1.withSelection(on, () => {
        var _a;
        const where = expression_builder_1.buildValue(filter);
...
function buildUnaryFilter(on, filter) {
    ...
    switch (op) {
        case 'IS NULL':
        case 'IS NOT NULL': {
            const leftValue = expression_builder_1.buildValue(operand);
            if (leftValue.index) {
...
function buildBinaryFilter(on, filter) {
    ...
    switch (op) {
        ...
        case 'IN':
        case 'NOT IN': {
            const value = expression_builder_1.buildValue(left);
            let arrayValue = expression_builder_1.buildValue(right);
        ...
        case 'LIKE': {
            ...
            if (value.index && value.index.expressions[0].hash === value.hash) {
                const valueToCompare = expression_builder_1.buildValue(right);
                if (valueToCompare.isConstant) {
                    const str = valueToCompare.get();
                    if (utils_1.nullIsh(str)) {
                        return new false_filter_1.FalseFilter(on);
                    }
                    ...
                    if (got) {
                        ...
                        return new seq_scan_1.SeqScanFilter(indexed, expression_builder_1.buildValue(filter));
                    }
                }
            }
        }
...
function buildComparison(on, filter) {
    ...
    let leftValue = expression_builder_1.buildValue(left);
    let rightValue = expression_builder_1.buildValue(right);
    if (leftValue.isConstant && rightValue.isConstant) {
        const global = expression_builder_1.buildValue(filter);
...
function buildTernaryFilter(on, filter) {
    switch (filter.op) {
        case 'BETWEEN':
        case 'NOT BETWEEN': {
            const value = expression_builder_1.buildValue(filter.value);
            const lo = expression_builder_1.buildValue(filter.lo);
            const hi = expression_builder_1.buildValue(filter.hi);
...
class StartsWithFilter extends transform_base_1.FilterBase {
    ...
    *enumerate(t) {
        ...
        for (const item of index.enumerate({
            ...
            if (utils_1.nullIsh(got) || !got.startsWith(this.startWith)) {
                break;
            }
            yield item;
        }
...
const lru_cache_1 = __importDefault(__webpack_require__(22));
const utils_1 = __webpack_require__(1);
const pg_utils_1 = __webpack_require__(97);
...
function replaceQueryArgs$(sql, values) {
    return sql.replace(/\$(\d+)/g, (str, istr) => {
        ...
        return pg_utils_1.toLiteral(val);
    });
}
class Adapters {
    constructor(db) {
...
var prepareValue = function (val, seen) {
    ...
    if (val instanceof Date) {
        ...
        // } else {
        return pg_escape_1.literal(dateToString(val));
        // }
    }
    if (Array.isArray(val)) {
        if (val.length === 0)
    ...
    return pg_escape_1.literal(val.toString());
};
...
function buildLimit(on, limit) {
    return context_1.withSelection(on, () => {
        const l = limit.limit && expression_builder_1.buildValue(limit.limit);
        const o = limit.offset && expression_builder_1.buildValue(limit.offset);
...
function buildDistinct(on, exprs) {
    return context_1.withSelection(on, () => {
        const vals = exprs && exprs.length > 0
            ? exprs.map(v => expression_builder_1.buildValue(v))
            : on.columns;
...
class OrderBy extends transform_base_1.FilterBase {
    constructor(selection, order) {
        super(selection);
        this.selection = selection;
        this.order = context_1.withSelection(selection, () => order.map(x => {
            var _a;
            return ({
                by: expression_builder_1.buildValue(x.by),
...
const datatypes_1 = __webpack_require__(3);
const interfaces_private_1 = __webpack_require__(0);
const pg_attribute_list_1 = __webpack_require__(104);
const pg_class_1 = __webpack_require__(105);
const pg_constraints_list_1 = __webpack_require__(106);
const pg_index_list_1 = __webpack_require__(107);
const pg_namespace_list_1 = __webpack_require__(108);
const pg_type_list_1 = __webpack_require__(109);
const functions_1 = __webpack_require__(110);
const pg_range_1 = __webpack_require__(115);
const expression_builder_1 = __webpack_require__(6);
const pg_database_1 = __webpack_require__(116);
const binary_operators_1 = __webpack_require__(117);
const sql_function_language_1 = __webpack_require__(118);
function setupPgCatalog(db) {
    ...
    binary_operators_1.registerCommonOperators(catalog);
    sql_function_language_1.registerSqlFunctionLanguage(db);
    catalog.setReadonly();
}
...
const interfaces_private_1 = __webpack_require__(0);
const datatypes_1 = __webpack_require__(3);
const table_index_1 = __webpack_require__(26);
const readonly_table_1 = __webpack_require__(9);
...
const string_1 = __webpack_require__(111);
const date_1 = __webpack_require__(112);
const system_1 = __webpack_require__(113);
const sequence_fns_1 = __webpack_require__(114);
...
exports.dateFunctions = [
    {
        ...
        implementation: (data, format) => {
            if (utils_1.nullIsh(data) || utils_1.nullIsh(format)) {
...
function getSeq(id) {
    const { transaction, schema } = utils_1.executionCtx();
    if (!transaction) {
...
exports.sequenceFunctions = [
    ...
    },
    {
        ...
        returns: datatypes_1.Types.integer,
        implementation: (seqId) => {
            const { transaction } = utils_1.executionCtx();
            if (!transaction) {
...
function registerNumericOperators(schema) {
    ...
    for (const [left, right, returns] of numberPairs()) {
        schema.registerOperator({
            ...
            right,
            returns,
            implementation: datatypes_1.isInteger(returns)
                ? ((a, b) => Math.trunc(a / b))
                : ((a, b) => a / b),
        });
    }
...
function registerDatetimeOperators(schema) {
    ...
    for (const dt of datatypes_1.dateTypes) {
        for (const [operator, f] of [['+', 1], ['-', -1]]) {
            schema.registerOperator({
                ...
                returns: dt,
                implementation: (a, b) => moment_1.default(a).add(f * utils_1.intervalToSec(b), 'seconds').toDate(),
            });
        }
    }
    ...
    for (const [operator, f] of [['+', 1], ['-', -1]]) {
        schema.registerOperator({
            ...
            implementation: (a, b) => moment_1.default(a).add(f * b, 'days').toDate(),
        });
    }
}
function registerJsonOperators(schema) {
    ...
    schema.registerOperator({
        ...
        implementation: (a, b) => utils_1.queryJson(b, a),
    });
...
function registerSqlFunctionLanguage(db) {
    db.registerLanguage('sql', ({ code, schema: _schema, args, returns: _returns }) => {
        ...
        // parse SQL
        const _parsed = parse_cache_1.parseSql(code);
        let parsed;
        ...
        const parameterList = evaluator_1.buildParameterList('', args);
        ...
        const executor = context_1.withParameters(parameterList, () => statement.compile());
        if (!(executor instanceof select_1.SelectExec)) {
            throw new interfaces_1.NotSupported(`Unsupported statement type in function: ${parsed.type}`);
        }
        ...
        const implem = (...args) => {
            ...
            return utils_1.pushExecutionCtx(exec, () => {
...
const columns_list_1 = __webpack_require__(120);
const table_list_1 = __webpack_require__(121);
